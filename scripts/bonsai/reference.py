#!/usr/bin/env python3
"""Numpy reference forward pass for Bonsai-1.7B from the manifest (P1 oracle).

Implements the full Qwen3 math on CPU (binary matmul, RMSNorm, q/k norm, YaRN RoPE
via the baked cos/sin caches, GQA attention, SwiGLU, lm_head) and checks it against
the golden logits. This becomes the exact spec the WGSL kernels must reproduce.

Usage: python reference.py --work <dir> [--golden <dir>]
"""
import argparse
import json
